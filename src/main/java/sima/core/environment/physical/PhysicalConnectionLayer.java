 ...
 * simulate different ways that how {@link sima.core.agent.SimaAgent} are physically connected. Thanks to this layer, we can for example simulate the
 * radio connection between {@link sima.core.agent.SimaAgent} and physically block the possibility to send of {@link PhysicalEvent} if two {@link
...
public abstract class PhysicalConnectionLayer {
     ...
     * method {@link #canBeSent(AgentIdentifier, AgentIdentifier, PhysicalEvent)} is called to verify if the decorated {@link PhysicalEvent} can be
     * sent to the target. If it is the case then verify if the {@link PhysicalConnectionLayer} has a next in the {@link PhysicalConnectionLayer}
     * chain. Call this method send to the next if there is a next, else call the method {@link #scheduleInEnvironment(AgentIdentifier, AgentIdentifier,
     * PhysicalEvent)} to schedule the {@link PhysicalEvent} via the {@link Environment} methods to send {@link sima.core.environment.event.Event}
     * like {@link Environment#assignEventOn(AgentIdentifier, AgentIdentifier, Event, long)}.
     *
     * <pre>{@code
    ...
    public void send(AgentIdentifier initiator, AgentIdentifier target, PhysicalEvent physicalEvent) {
        ...
        if (hasNext() && success)
            ...
            scheduleInEnvironment(initiator, target, decoratedPhysicalEvent);
        // else stop the chain.
    }

    /**
     * Allow the user to decorate the {@link PhysicalEvent} specified in the method {@link #send(AgentIdentifier, AgentIdentifier, PhysicalEvent)}.
     *
     ...
     * AgentIdentifier, PhysicalEvent)} and can do more than just verify if both {@link sima.core.agent.SimaAgent} have physical connection with the
     * method {@link #hasPhysicalConnection(AgentIdentifier, AgentIdentifier)}.
     *
     * @param initiator     the initiator of the {@link PhysicalEvent}
     * @param target        the target of the {@link PhysicalEvent}
     * @param physicalEvent the {@link PhysicalEvent}
     ...
     * Environment#assignEventOn(AgentIdentifier, AgentIdentifier, Event, long)}
     *
     * @param initiator     the agent which initiates the event
     * @param target        the agent which must receive the {@link PhysicalEvent}
     * @param physicalEvent the {@link PhysicalEvent}
     */
    protected abstract void scheduleInEnvironment(AgentIdentifier initiator, AgentIdentifier target, PhysicalEvent physicalEvent);
