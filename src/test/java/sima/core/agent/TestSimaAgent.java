...
public class TestSimaAgent {
     ...
     */
    protected void simulationWithoutEnvironment(
     ...
     * @param agentIdentifier            the agent identifier
     */
    protected void simulationWithEnvironment(
            MockedStatic<SimaSimulation> simaSimulationMockedStatic, AgentIdentifier agentIdentifier) {
        simaSimulationMockedStatic.when(() -> SimaSimulation.getAgentEnvironment(agentIdentifier))
                .then(invocation -> {
    ...
    class StartTest {
        ...
        void testStartAfterKill() {
            try (MockedStatic<SimaSimulation> simaSimulationMockedStatic = mockSimaSimulation()) {
                // GIVEN
                simulationWithoutEnvironment(simaSimulationMockedStatic,
        ...
        @Test
        @DisplayName("Yest if the method start throws an AlreadyStartedAgentException if the SimpleAgent is already " +
                "started")
        ...
        @Test
        @DisplayName("Test if start notifies all protocol added to the agent")
        void testStartNotifiesAllProtocols() {
            String tag0 = "TAG0";
            String tag1 = "TAG1";
            simaAgent.addProtocol(CorrectProtocol0.class, tag0, null);
            simaAgent.addProtocol(CorrectProtocol0.class, tag1, null);

            simaAgent.start();

            CorrectProtocol0 p0 = (CorrectProtocol0) simaAgent.getProtocol(new ProtocolIdentifier(CorrectProtocol0.class, tag0));
            CorrectProtocol0 p1 = (CorrectProtocol0) simaAgent.getProtocol(new ProtocolIdentifier(CorrectProtocol0.class, tag1));

            assertThat(p0.getOnOwnerStart()).isEqualByComparingTo(1);
            assertThat(p1.getOnOwnerStart()).isEqualByComparingTo(1);
        }
    ...
    class KillTest {
        ...
        void testKillWithOneEnvironment() {
            try (MockedStatic<SimaSimulation> simaSimulationMockedStatic = mockSimaSimulation()) {
                // GIVEN
                simulationWithEnvironment(simaSimulationMockedStatic,
                                          simaAgent.getAgentIdentifier());
        ...
        @Test
        @DisplayName("Test if kill notifies all protocol added to the agent")
        void testKillNotifiesAllProtocols() {
            try (MockedStatic<SimaSimulation> simaSimulationMockedStatic = mockSimaSimulation()) {
                // GIVEN
                simulationWithEnvironment(simaSimulationMockedStatic,
                                          simaAgent.getAgentIdentifier());
                ...
                assertThat(p0.getOnOwnerKill()).isEqualByComparingTo(1);
                assertThat(p1.getOnOwnerKill()).isEqualByComparingTo(1);
            }
        }
    }
